// src/screens/reports/CategoriesReportPage.tsx
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="rounded-md border border-kk-dark-input-border bg-kk-dark-bg p-4 shadow-sm">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs text-kk-dark-text-muted">Category scope</label>
            <select
              className="rounded-md border border-kk-dark-input-border bg-kk-dark-bg px-3 py-2 text-sm"
              value={groupBy}
            ...
            >
              ...
              <option value="all">All categories (parents + children)</option>
            </select>
          </div>
        </div>
      </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={lineData} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                ...
                <Tooltip
                  formatter={(v: any) => formatMoneyNGN(Number(v))}
      ...
      <div className="rounded-md border border-kk-dark-input-border bg-kk-dark-bg shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-kk-dark-input-border">
          ...
          <div className="text-xs text-kk-dark-text-muted">
            Net sales, order count, and product count per category.
          </div>
        </div>
              ...
              {loading ? (
                ...
                tableRows.map((r) => (
                  <tr
                    key={r.category_id}
                    className={[
                      "border-t border-kk-dark-border",
                      groupBy === "top_level" && r.category_id !== 0 ? "bg-kk-dark-bg-elevated cursor-pointer" : "",
                    ].join(" ")}
                    onClick={() => {
                      if (groupBy !== "top_level") return;
                      if (!r.category_id || r.category_id === 0) return;
                      nav(`/reports/categories/${r.category_id}?start=${start}&end=${end}`);
                    }}
                    title={groupBy === "top_level" && r.category_id !== 0 ? "View child categories" : undefined}
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-right">
                      {formatMoneyNGN(Number(r.net_sales ?? 0))}
                    </td>
                    <td className="px-4 py-3 text-sm text-right">{formatNumber(r.orders ?? 0)}</td>
                    <td className="px-4 py-3 text-sm text-right">
                      {formatNumber(Number(r.items_sold ?? 0))}
                    </td>
                  </tr>
                ))
              ) : (
