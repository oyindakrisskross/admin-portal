...
import { fetchMonthlyReportSettings, runMonthlyReports, updateMonthlyReportSettings } from "../../api/reports";
import type { Outlet } from "../../types/location";
import type { MonthlyReportRunResult, MonthlyReportSettings } from "../../types/monthlyReports";

const SETTINGS_CACHE_KEY = "kk.monthlyReports.settings.v1";
...
function loadCachedSettings(): MonthlyReportSettings | null {
  try {
    const raw = localStorage.getItem(SETTINGS_CACHE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object") return null;
    return parsed as MonthlyReportSettings;
  } catch {
    return null;
  }
}

function saveCachedSettings(cfg: MonthlyReportSettings) {
  try {
...
export default function MonthlyReportsSettingsPage() {
  const today = useMemo(() => new Date(), []);
  const [outlets, setOutlets] = useState<Outlet[]>([]);
  const [cfg, setCfg] = useState<MonthlyReportSettings | null>(null);
  const [loadedFromCache, setLoadedFromCache] = useState(false);

  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [running, setRunning] = useState(false);
  const [runMonth, setRunMonth] = useState<string>(toYM(today));
  const [runResults, setRunResults] = useState<MonthlyReportRunResult[] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const s = await fetchMonthlyReportSettings();
        if (cancelled) return;
        setCfg(s);
        ...
        if (fallback) {
          ...
          setLoadedFromCache(true);
          showToast(
            "Loaded cached monthly report settings (server not reachable). Save again to sync.",
            "info"
          );
        } else {
          showToast(e?.message ?? "Failed to load monthly report settings");
          setCfg(null);
        }
      } finally {
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      const patch: Partial<MonthlyReportSettings> = {
        enabled: cfg.enabled,
      ...
      const next = await updateMonthlyReportSettings(patch);
      setCfg(next);
      setLoadedFromCache(false);
      saveCachedSettings(next);
      showToast("Monthly report settings saved.", "success");
    } catch (e: any) {
  ...
  const onRunNow = async () => {
    ...
    setRunResults(null);
    try {
      const resp = await runMonthlyReports({
        month: runMonth,
      ...
      if (failed.length) {
        ...
        showToast("Monthly reports generated successfully.", "success");
      }
    } catch (e: any) {
  ...
  return (
    <div className="p-6 space-y-6">
      <ListPageHeader title="Monthly Reports" subtitle="Auto-generate per-location monthly sales reports." />
          ...
          <>
            ...
            {loadedFromCache && (
              <div className="text-xs text-amber-300">
                Using cached settings from this browser. Click "Save Settings" to sync to the server when it is
                available.
              </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder={cfg.export_backend === "GRAPH" ? "KrissKross Reports" : "Monthly Reports"}
                />
              </div>
            </div>
      ...
      <div className="rounded-md border border-kk-dark-border bg-kk-dark-bg-elevated p-4 space-y-3">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end">
          <div className="space-y-1">
            <label className="text-xs text-kk-dark-text-muted">Month</label>
            <input
              type="month"
              value={runMonth}
        ...
        {runResults && (
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {runResults.map((r) => (
                  <tr key={`${r.location_id}-${r.report_month}`} className="border-b border-kk-dark-border">
                    <td className="py-2 pr-4">{r.location_name}</td>
                    <td className="py-2 pr-4">{r.report_month}</td>
