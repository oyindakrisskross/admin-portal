...
export function SplitFlapValue({ value, className }: SplitFlapValueProps) {
  ...
  return (
    <span className={className}>
      ...
      <span className="kk-slot-value" aria-hidden="true">
        {toChars.map((toChar, index) => {
          const fromChar = fromChars[index] ?? " ";
          const digitChar = isDigit(toChar);
          const shouldSpin = !!transition && isDigit(toChar) && fromChar !== toChar;
          ...
          if (!shouldSpin) {
            return (
              <span
                key={`${index}-static`}
                className={`kk-slot-char ${digitChar ? "kk-slot-char-digit" : "kk-slot-char-symbol"}`}
              >
                {displayTo}
              </span>
            );
          ...
          const reelStyle = {
            "--kk-slot-steps": String(steps),
            "--kk-slot-duration": `${DIGIT_SPIN_DURATION_MS}ms`,
            "--kk-slot-delay": `${delayMs}ms`,
          } as CSSProperties;

          return (
            <span key={`${index}-${transition.key}`} className="kk-slot-char kk-slot-char-digit kk-slot-char-reel">
              <span className="kk-slot-reel-window">
                <span className="kk-slot-reel-strip kk-slot-reel-strip-spinning" style={reelStyle}>
                  {strip.map((digit, digitIndex) => (
                    <span key={`${index}-${transition.key}-${digitIndex}`} className="kk-slot-reel-digit">
                      {digit}
                    </span>
                  ))}
                </span>
